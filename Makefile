...
pkgdatadir = $(datadir)/gobytecore
pkgincludedir = $(includedir)/gobytecore
pkglibdir = $(libdir)/gobytecore
pkglibexecdir = $(libexecdir)/gobytecore
...
CONFIG_HEADER = $(top_builddir)/src/config/gobyte-config.h
CONFIG_CLEAN_FILES = libgobyteconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libgobyteconsensus.pc.in \
	...
	$(top_srcdir)/src/config/gobyte-config.h.in \
...
ACLOCAL = ${SHELL} /home/gobyte/gobytes-test/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/gobyte/gobytes-test/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/gobyte/gobytes-test/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/gobyte/gobytes-test/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /home/gobyte/gobytes-test/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = gobytecore
PACKAGE_BUGREPORT = https://github.com/gobytecoin/gobyte/issues
PACKAGE_NAME = GoByte Core
PACKAGE_STRING = GoByte Core 0.12.2
PACKAGE_TARNAME = gobytecore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/gobyte/gobytes-test
abs_srcdir = /home/gobyte/gobytes-test
abs_top_builddir = /home/gobyte/gobytes-test
abs_top_srcdir = /home/gobyte/gobytes-test
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/gobyte/gobytes-test/build-aux/install-sh
...
pkgconfig_DATA = libgobyteconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/gobyted$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/gobyte-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/gobyte-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = GoByte-Qt.app
OSX_DMG = GoByte-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_gobyte_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_gobyte_coverage.info test_gobyte.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/GoByte-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/gobyte-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/gobyte-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/gobyte-config.h
$(top_srcdir)/src/config/gobyte-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/gobyte-config.h src/config/stamp-h1
libgobyteconsensus.pc: $(top_builddir)/config.status $(srcdir)/libgobyteconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/GoByte-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/GoByte-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "GoByte-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/GoByte-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_gobyte.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_gobyte -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_gobyte_filtered.info: test_gobyte.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_gobyte_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_gobyte_filtered.info
...
#test_gobyte_coverage.info: baseline_filtered_combined.info test_gobyte_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_gobyte_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_gobyte_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_gobyte_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_gobyte.coverage/.dirstamp:  test_gobyte_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_gobyte.coverage/.dirstamp total.coverage/.dirstamp

# GoByte: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_gobyte.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
